...
export interface FundingMethod {
  ...
  /** Fee as a percentage (0–1), or flat description */
  feeLabel:     string
  minAmount:    number   // USD
  maxAmount:    number   // USD
  isAvailable:  boolean
  iconLabel:    string   // short label for icon badge
}

export const MOCK_FUNDING_METHODS: FundingMethod[] = [
  {
    id:             'usdc-polygon',
...
export function getFundingMethod(id: FundingMethodId): FundingMethod | undefined {
  return MOCK_FUNDING_METHODS.find((m) => m.id === id)
}

export function getFundingMethodsByCategory(category: FundingMethodCategory): FundingMethod[] {
  return MOCK_FUNDING_METHODS.filter((m) => m.category === category)
}
